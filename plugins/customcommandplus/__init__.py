...
class CustomCommandPlus(_PluginBase):
    ...
    def __execute_command(self, name, command, random_delay=None):
        ...
        if self._notify and self._msgtype:
            ...
            if self._msgtype:
                mtype = NotificationType.__getitem__(
                    str(self._msgtype)) or NotificationType.Manual

            if result_obj["status"] == "success":
                title = name + " ✅"
            elif result_obj["status"] == "fail":
                title = name + " ✴️"
            else:
                title = name
    ...
    def __load_result(self, last_output):
        if last_output:
            try:
                result_obj = json.loads(last_output)
